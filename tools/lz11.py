...
def main() -> int:
    ...
    parser.add_argument("--reject-trailing", action="store_true")
    args = parser.parse_args()

    result = decompress_lz11(args.input.read_bytes(), allow_trailing=not args.reject_trailing)
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_bytes(result.output)
    print(f"decoded={len(result.output)} consumed={result.consumed} trailing={len(result.trailing)}")
    return 0
