...
| [Public Source Sweep 01](research/PUBLIC_SOURCE_SWEEP_01.md) | Census-first rule: exhaust public source families before allowing any single tool to define the project model |
...
| [Public Source Inventory](../manifests/source-inventory.csv) | Machine-readable registry of individual reviewed, located, pending, and access-limited public sources |
| [Source-Family Coverage](../manifests/source-coverage.csv) | Exhaustive coverage matrix showing which public-source families are started, missing, or blocked |
| [Regional Official Sources](../manifests/regional-official-sources.csv) | Japan-origin regional/locale official-source inventory, including explicit missing-source gaps |
| [pk3DS Index 01](research/PK3DS_INDEX_01_CORE_DATA.md) | First pinned source-level audit of Gen VI/ORAS game config, GARC mapping, personal/move/learnset/evolution/trainer structures |
...
`research/` now contains source-level audits pinned to specific upstream revisions plus census/sweep records. A research index records what a public implementation or document says; it does not automatically promote that claim to direct retail observation.

## Recommended documentation flow

1. Identify the target in `VERSIONS.md`.
2. Expand and check `../manifests/source-coverage.csv`; do not narrow research to one implementation while major source families remain unenumerated.
3. Register each located source in `../manifests/source-inventory.csv`, and regional official evidence in `../manifests/regional-official-sources.csv` where appropriate.
4. Follow `RESEARCH_POLICY.md` / `RESEARCH_GUIDE.md` and record source-level audits in `research/` or format-specific notes in `formats/`.
...
8. Update `PROJECT_STATUS.md`, `VERSIONS.md`, and `ROADMAP.md` when meaningful milestones are reached.
...
- A source-family landing page being located does not mean that family is surveyed; enumerate relevant pages, threads, repositories, historical forks, captures, and linked documents as far as practicable.
