...
This document marks the transition from repository setup into active Pokémon Omega Ruby decompilation work.
...
Do not assume that a layout from Alpha Sapphire, X, Y, or another 3DS title is identical.
...
Pokémon Omega Ruby is reconstructed as its own verified target first. Cross-title comparison uses explicit buckets:

- **ORAS common** — independently verified in both Omega Ruby and Alpha Sapphire
...
XY behavior is not projected onto ORAS, and ORAS additions are not projected backward onto XY without direct verification.
...
- Verified Pokémon Omega Ruby retail target manifest: pending a locally available source
