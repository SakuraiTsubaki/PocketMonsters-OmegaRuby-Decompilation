...
**Public-research baseline; not yet verified against a local Pokémon Omega Ruby dump.**
...
Public Gen VI research describes this family as inherited from earlier Game Freak titles and still used in the Generation VI engine family.
...
Until a real sample is available, the parser must support synthetic test fixtures and must not claim byte-perfect Pokémon Omega Ruby compatibility.
...
7. compare OR/AS with X/Y rather than assuming all engine-family details are identical.
